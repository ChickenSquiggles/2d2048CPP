...
void Game::init(const char *title, int xpos, int ypos, int width, int height, bool fullscreen)
{
	...
	SDL_Surface* GameOverTile = IMG_Load("Assets/GameOver.png");
	...
	gameover = SDL_CreateTextureFromSurface(renderer, GameOverTile);
...
int checkforover()
{
	...
	for (i = 0; i < 3; i++)
		for (j = 0; j < 3; j++)
			if (newarray[i + 1][j] == newarray[i][j] || newarray[i][j + 1] == newarray[i][j])
			{
				gl = 1;
				break;
			}
...
void Game::render()
{
	...
	if (!templost)
	{
		if (!checkforover())
		{
			...
			destR.w = 460;
			destR.h = 460;
			SDL_RenderCopy(renderer, gameover, NULL, &destR);
			lost = true;
